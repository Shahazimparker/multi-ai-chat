...
const sendMessage = async (req, res) => {
  ...
  // Only connect to business DB when dbOnly mode is active
  if (effectiveDbOnly && bizDbConnected === null) {
  ...
  try {
    ...
    */
    ...
    // dbOnly=true  → minimal schema (table guide + FK relationships) — AI uses GET_SCHEMA for columns
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      // Supports formats: [GET_SCHEMA:tables], <GET_SCHEMA>tables</GET_SCHEMA>, and <request_label>Get Schema</request_label> <request_text>tables</request_text>
      let getSchemaMatch = reply.match(/\[GET_SCHEMA:([^\]]+)\]/);
      if (!getSchemaMatch) {
        getSchemaMatch = reply.match(/<GET_SCHEMA>([^<]+)<\/GET_SCHEMA>/);
      }
      if (!getSchemaMatch) {
        getSchemaMatch = reply.match(/<request_label>Get\s+Schema<\/request_label>\s*<request_text>([^<]+)<\/request_text>/i);
      }
      ...
      //   <QUERY_DB>sql</QUERY_DB>
      //   <query>sql</query>
      //   <Function id="query_db_1">sql</Function>
      ...
      // Detect bare closing tag ([/QUERY_DB], </query>, or </Function>) without matching opening tag or SQL
      // AI sometimes outputs "text... [/QUERY_DB]" thinking it opened the tag earlier
      if (!queryDbMatch) {
        const hasBareCloseQueryDb = /\[\/QUERY_DB\]/.test(reply) && !/\[QUERY_DB\]/.test(reply);
        const hasBareCloseQuery = /<\/query>/i.test(reply) && !/<query>/i.test(reply);
        const hasBareCloseFunction = /<\/Function>/i.test(reply) && !/<Function\s+id="query_db_\d+"\s*>/i.test(reply);
        if (hasBareCloseQueryDb || hasBareCloseQuery || hasBareCloseFunction) {
          aiMessages.push({ role: 'assistant', content: reply });
          aiMessages.push({
            role: 'user',
...
Write the SQL query now inside [QUERY_DB] and [/QUERY_DB] tags. Do not use closing tags alone.`
        ...
        try {
          ...
          if (resultCount === 0) {
            ...
            if (consecutiveZeroResults >= 4) {
              // Fourth consecutive zero — stop retrying, finalize with "no data"
              finalReply = reply.replace(queryDbMatch[0], '').trim() || `No data found.`;
              console.log(`[Tool] Second consecutive zero-result — breaking tool loop`);
              break;
            }
          ...
          const dbStatusNote = resultCount === 0
            ? 'No rows returned for this query.'
            : '';

          aiMessages.push({
            role: 'assistant',
            content: reply.replace(queryDbMatch[0], '').trim() || dbStatusNote,
          });
    ...
    // Strip any leftover tool-call tags that AI leaked into the final reply
    finalReply = finalReply
      .replace(/\[QUERY_DB\]\s*(?:<SQL_QUERY>\s*)?[\s\S]*?\s*(?:\[\/QUERY_DB\]|<\/SQL_QUERY>|<\/QUERY_DB>)/g, '')
      .replace(/<QUERY_DB>\s*(?:<SQL_QUERY>\s*)?[\s\S]*?\s*(?:<\/SQL_QUERY>\s*)?[\s\S]*?\[\/QUERY_DB\]/g, '') // mixed <QUERY_DB> ... </SQL_QUERY> [/QUERY_DB]
    ...
    // Skip if we intentionally broke due to consecutive zero results
