...
- **Trace & Log Analysis (measured, not eyeballed)**: Keyword scanning cannot find a slow query, because a slow query is a number rather than a word. Uploaded spreadsheets and logs additionally get:
  - a **column census** stating raw facts and no meaning, so the model reads the columns itself — headers in any language, or none at all — and then asks for exact computation over every row (`[ANALYZE_TABLE]`, with `where`/`min`/`max` filters)
  - **Drain** log-template mining ([He et al., ICWS 2017](https://jiemingzhu.github.io/pub/pjhe_icws2017.pdf) — the parser behind logpai/Drain3 and Salesforce LogAI), which isolates **rare events**: in a log where one line repeats 40,000 times, the line that occurred once is usually the incident. A 13.3 MB / 200k-line log collapses to 6 templates in ~755 ms
  - **burst detection** over time buckets (median + 4×MAD), so an error is located to a moment rather than to a file
  - SQL fingerprinting at [Percona `pt-fingerprint`](https://docs.percona.com/percona-toolkit/pt-fingerprint.html) parity, which is what turns 39,000 "unrelated" lookups into one N+1
  - `[READ_ROWS]` to read the lines at any located position, and `[COMPARE_FILES]` to diff a working baseline against a failing run — an event that **stopped** appearing is often the failure itself
  - Every computed section declares how it read the file; anything inferred is labelled `GUESSED` / `UNVERIFIED` rather than presented as measurement
  - Validate against a real export with `npm --prefix backend run inspect -- <path>` (reads from disk: no upload, no API key, no cost)
...
- **Raw Prompt Preservation & Measured Context Window**: Query compression disabled across all models. The assembled prompt is measured against the model window and sent untouched when it fits — a 60-turn conversation on a 128K model loses nothing. On genuine overflow, whole history turns are evicted oldest-first (never mid-message) down to a low-water mark, so the prompt prefix stays stable for several turns. Flat 8,192 reserved output tokens for $\ge 32\text{k}$ models plus a 6% safety margin.
- **Provider Prompt Caching**: Retrieved context travels with the question rather than ahead of the history, keeping system + history a byte-identical cacheable prefix (~1K -> ~85K tokens on a long thread). Verified live: 2,432 cached tokens on DeepSeek, 1,152 on Mistral. Mistral requires `prompt_cache_key` (it does not cache without it); Claude gets an explicit breakpoint at the end of the history. Cache reads/writes are recorded per turn and surfaced in the admin dashboard.
...
- **Authenticated chat flows** with `httpOnly` cookie auth, double-submit CSRF, and a 30-minute idle logout that is **skipped for "Remember me" sessions** (those run the full 30 days on their own cookie expiry)
