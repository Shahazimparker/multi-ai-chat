-- ============================================================
-- FILE: database/schema.sql (UPDATED WITH FILE UPLOAD)
-- PURPOSE: Complete Supabase/PostgreSQL schema for MultiAI Chat
-- WITH: File uploads, RAG, attachments support
...
-- ─────────────────────────────────────────────
-- TABLE: uploaded_files (NEW - user files)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- TABLE: rag_chunks (NEW - searchable chunks)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- TABLE: query_analytics (all user queries)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- TABLE: rag_documents (original RAG docs)
-- ─────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS uploaded_files_topic_id_idx
  ON uploaded_files(topic_id);
...
-- ─────────────────────────────────────────────
-- FUNCTION: search_uploaded_files (NEW)
...
BEGIN
  ...
  WHERE uf.user_id = user_id_param
    AND 1 - (rc.embedding <=> query_embedding) > 0.5
  ORDER BY similarity DESC
  LIMIT match_count;
END;
$$;
...
-- password_hash = bcrypt of 'Admin@1234'
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- COMMENTS: What each table does
-- ─────────────────────────────────────────────

-- users: App users (created by admin)
-- sessions: JWT token tracking
-- topics: Chat conversations (one per topic)
-- messages: Individual chat messages in a topic
-- uploaded_files: User's uploaded PDF/image/doc/txt files
-- rag_chunks: Chunks of uploaded files (for vector search)
-- query_cache: Cache of AI responses for repeated queries
-- query_analytics: All user queries for admin analytics
-- rag_documents: Original knowledge base documents